...
/** NSD 优先，失败或并行用 UDP PTVDISC1；结果按 host 去重。 */
        ...
        val disc = object : NsdManager.DiscoveryListener {
            ...
            override fun onServiceFound(serviceInfo: NsdServiceInfo) {
                if (serviceInfo.serviceType != Constants.NSD_TYPE &&
                    !serviceInfo.serviceType.contains("pockettv")
                ) {
                    return
                }
                mgr.resolveService(
        ...
        Thread(
            {
                ...
                try {
                    ...
                    multicast = wifi?.createMulticastLock("pockettv-disc")
